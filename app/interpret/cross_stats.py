...
from app.db import fetch_industry_board_close_heat, fetch_trading_dates_ending

_EMPTY = {"macd_up": 0, "macd_down": 0, "heat_up": 0, "heat_down": 0}
_HISTORY_DAYS = 120


def attach_cross_stats(reading: dict, as_of: date) -> dict:
    ...
    counts = recent_cross_counts(codes, as_of)
    sections = []
    for section in reading.get("sections") or []:
        rows = []
        for row in section.get("rows") or []:
...
def recent_cross_counts(
    ...
    recent = fetch_trading_dates_ending("industry", as_of, days)
    if not board_codes or not recent:
        return {}
    history = fetch_trading_dates_ending("industry", as_of, _HISTORY_DAYS)
    start = history[0] if history else recent[0]
    series = fetch_industry_board_close_heat(list(dict.fromkeys(board_codes)), start, as_of)
